...
/**
 * *****************************************************************************************
 * _draggable_leftangle - Draggable leftangle component
 * *****************************************************************************************
 * PLEASE NOTE: This component uses the generic highlighter (Outside highlighter for 
 * joining and resizing the leftangle)) as well as a custom highlighter (at the leftangle mid point
 * useful for dragging the leftangle around).)
 * *****************************************************************************************
 * ***************************************************************************************** 
 * @param {int} id - leftangle ID
 * @param {float} start_pos_x - leftangle start x position
 * @param {float} start_pos_y - leftangle start y position
 * @param {float} end_pos_x - leftangle end x position
 * @param {float} end_pos_y - leftangle end y position
 * @param {string} colour - leftangle colour
 * @param {int} stroke_width - leftangle stroke width
 * @param {boolean} is_highlighted - leftangle highlighted state
 * @param {function} taskboard_rerender_func - Function to trigger taskboard re-render
 * @param {boolean} show_toolbar - leftangle toolbar visibility
...
const _draggable_leftangle = ({ 
    ...
    const [leftangle_start_pos, _set_leftangle_start_pos] = useState({ x: start_pos_x, y: start_pos_y });
    const [leftangle_end_pos, _set_leftangle_end_pos] = useState({ x: end_pos_x, y: end_pos_y });
    ...
    const [is_dragging_leftangle, _set_is_dragging_leftangle] = useState(false); 
    const [prevent_leftangle_deactivation, _set_prevent_leftangle_deactivation] = useState(false); 
    const [joining_show_highlighter, _set_joining_show_highlighter] = useState(false); 
    const [local_param_leftangle_active, _set_local_param_leftangle_active] = useState(true); // local param to track leftangle active state
    ...
    const leftangle_root_ref = useRef(null);
    
    const HLIGHT_CIRC_RADIUS_RATIO_TO_STROKEWIDTH   = 0.02;
    const TOOLBAR_DISTANCE_TOP_PERC                 = 0.1; // percentage of the window height
    const RECT_FILLET_PERCENTAGE                    = 0.2; // percentage of the leftangle width/height for fillet radius

    
    // Update leftangle props when they are changed change
    useEffect(() => {
        if(is_dragging_leftangle === false)
        {
            _set_leftangle_start_pos({ x: start_pos_x, y: start_pos_y });
            _set_leftangle_end_pos({ x: end_pos_x, y: end_pos_y });
        }
    }, [start_pos_x , start_pos_y, end_pos_x, end_pos_y]);

    const _deactivate_leftangle = (event_target) => { 
        // e.preventDefault();  // might be needed for some other component (need to pass in the event to use this
                                // not the event target)

        if(prevent_leftangle_deactivation === false)
        {
            _set_arr_highlighted(false);
            _update_leftangle_highlighted(id, false);
            _set_display_toolbar(false);
            _update_leftangle_toolbar_show(id, false);
            taskboard_rerender_func();

            _set_local_param_leftangle_active(false);
        }
        else
        {
            _set_prevent_leftangle_deactivation(false);
        }
    };
    ...
    const _get_rect_diagonal_length = () => {
        const dx = leftangle_end_pos.x - leftangle_start_pos.x;
        const dy = leftangle_end_pos.y - leftangle_start_pos.y;
        return Math.sqrt(dx * dx + dy * dy);
    };

    const _toolbar_item_clicked_notif = (leftangle_tb_item_index) => {
        switch(leftangle_tb_item_index)
        {
    ...
    const _update_colour = (updated_hex_colour_val) => {
        _update_leftangle_colour(id, updated_hex_colour_val);
        taskboard_rerender_func();
    };
    ...
    const _get_toolbar_position = () => {
        ...
        let x = (leftangle_start_pos.x + leftangle_end_pos.x) / 2;
        let y = 0; 
        
        if(leftangle_start_pos.y < leftangle_end_pos.y)
        {
            y = leftangle_start_pos.y - toolbar_distance_top;
        }
        else
        {
            y = leftangle_end_pos.y - toolbar_distance_top;
        } 
    ...
    const _highlighter_drag_mouse_down = (drag_direction) => {
        _set_prevent_leftangle_deactivation(true);
        taskboard_rerender_func();
    ...
    const _highlighter_drag_mouse_up = (drag_direction, width_increase_pixels, height_increase_pixels) => 
    {
        ...
        switch(drag_direction)
        {
            case HIGHLIGHT_DRAG_DIRECTION.BOTTOM_RIGHT:
            {
                ...
                _update_leftangle_end_pos(id, leftangle_end_pos.x + width_increase_pixels, leftangle_end_pos.y + height_increase_pixels, true);          
                taskboard_rerender_func();
                break;
            }
            ...
            {
                // calculate new width and height
                _update_leftangle_start_pos(id, leftangle_start_pos.x - width_increase_pixels, leftangle_start_pos.y - height_increase_pixels, true);
                taskboard_rerender_func();
                break;
            }
    ...
    const _on_mouse_down = (e) => {
        ...
        _update_leftangle_toolbar_show(id, true);
        taskboard_rerender_func();
    };
    ...
    useEffect(() => {
        if (leftangle_root_ref.current && !leftangle_root_ref.current.contains(main_page_last_click_event_target)) {
            // clicked outside of the leftangle component, deactivate leftangle  
            if(local_param_leftangle_active === true)
            {
                _deactivate_leftangle(main_page_last_click_event_target);
            }
        }
        ...
        const _handle_mouse_up = (event) => {   
            if(is_dragging_hlighter === true)
            {
                if(selected_hlight_pos === LINE_HLIGHT_DRAG_POS.MID)
                {
                    ...
                    const mid_x = (leftangle_start_pos.x + leftangle_end_pos.x) / 2;
                    const mid_y = (leftangle_start_pos.y + leftangle_end_pos.y) / 2;
                    const dx = clientX - mid_x;
                    const dy = clientY - mid_y;

                    _set_leftangle_start_pos({x: leftangle_start_pos.x + dx, y: leftangle_start_pos.y + dy});
                    _set_leftangle_end_pos({x: leftangle_end_pos.x + dx, y: leftangle_end_pos.y + dy});
                    _set_is_dragging_hlighter(false);
                    _update_leftangle_start_pos(id, leftangle_start_pos.x + dx, leftangle_start_pos.y + dy, true);
                    _update_leftangle_end_pos(id, leftangle_end_pos.x + dx, leftangle_end_pos.y + dy, true);
                }
        ...
        const _handle_mouse_move = (event) => {   
            if(is_dragging_hlighter === true)
            {
                if(selected_hlight_pos === LINE_HLIGHT_DRAG_POS.MID)
                {
                    ...
                    _set_leftangle_start_pos({x: leftangle_start_pos.x + dx, y: leftangle_start_pos.y + dy});
                    _set_leftangle_end_pos({x: leftangle_end_pos.x + dx, y: leftangle_end_pos.y + dy});
                    _update_leftangle_start_pos(id, leftangle_start_pos.x + dx, leftangle_start_pos.y + dy, false);
                    _update_leftangle_end_pos(id, leftangle_end_pos.x + dx, leftangle_end_pos.y + dy, false);
                }
    ...
    // calculate leftangle top left position
    const overall_top_left = {
        x: Math.min(leftangle_start_pos.x, leftangle_end_pos.x),
        y: Math.min(leftangle_start_pos.y, leftangle_end_pos.y)
    };

    return (
        <div 
            ref={leftangle_root_ref} 
            id="leftangle_root"
            onMouseEnter={(e) => { _on_mouse_hover(e); }}    
            onMouseDown = {(e) => { _on_mouse_down(e); }}
        >
            {/* display leftangle toolbar */}
            <div>
            ...
            <div>
                {(is_highlighted === true || joining_show_highlighter === true) ? (
                    <_highlighter 
                        ...
                        item_width={Math.abs(leftangle_end_pos.x - leftangle_start_pos.x)} 
                        item_height={Math.abs(leftangle_end_pos.y - leftangle_start_pos.y)} 
                        z_index={z_index} 
            ...
            <div>
                ...
                >
                    ...
                    <polygon
                        points={`
                            ${Math.max(leftangle_start_pos.x, leftangle_end_pos.x)},${Math.max(leftangle_start_pos.y, leftangle_end_pos.y)}
                            ${Math.min(leftangle_start_pos.x, leftangle_end_pos.x)},${Math.max(leftangle_start_pos.y, leftangle_end_pos.y)}
                            ${Math.max(leftangle_start_pos.x, leftangle_end_pos.x)},${Math.min(leftangle_start_pos.y, leftangle_end_pos.y)}
                        `}
                        fill="none"
                        stroke={colour}
                    ...
                    {(arr_highlighted === true) ? (
                        <circle 
                            className="highlighter_leftangle"
...
export default _draggable_leftangle;
