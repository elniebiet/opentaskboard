...
/**
 * *****************************************************************************************
 * _draggable_rectangle - Draggable rectangle component
 * *****************************************************************************************
 * PLEASE NOTE: This component uses the generic highlighter (Outside highlighter for 
 * joining and resizing the rectangle)) as well as a custom highlighter (at the rectangle mid point
 * useful for dragging the rectangle around).)
 * *****************************************************************************************
 * ***************************************************************************************** 
 * @param {int} id - rectangle ID
 * @param {float} start_pos_x - rectangle start x position
 * @param {float} start_pos_y - rectangle start y position
 * @param {float} end_pos_x - rectangle end x position
 * @param {float} end_pos_y - rectangle end y position
 * @param {string} colour - rectangle colour
 * @param {int} stroke_width - rectangle stroke width
 * @param {boolean} is_highlighted - rectangle highlighted state
 * @param {function} taskboard_rerender_func - Function to trigger taskboard re-render
 * @param {boolean} show_toolbar - rectangle toolbar visibility
 ...
 * @param {boolean} active - rectangle active state
...
const _draggable_rectangle = ({ 
    ...
    const [is_dragging_rectangle, _set_is_dragging_rectangle] = useState(false); 
    const [prevent_rectangle_deactivation, _set_prevent_rectangle_deactivation] = useState(false); 
    const [joining_show_highlighter, _set_joining_show_highlighter] = useState(false); 
    const [local_param_rectangle_active, _set_local_param_rectangle_active] = useState(true); // local param to track rectangle active state
    ...
    useEffect(() => {
        if(is_dragging_rectangle === false)
        {
            _set_rectangle_start_pos({ x: start_pos_x, y: start_pos_y });
            _set_rectangle_end_pos({ x: end_pos_x, y: end_pos_y });
        }
    }, [start_pos_x , start_pos_y, end_pos_x, end_pos_y]);

    const _deactivate_rectangle = (event_target) => { 
        // e.preventDefault();  // might be needed for some other component (need to pass in the event to use this
                                // not the event target)

        if(prevent_rectangle_deactivation === false)
        {
            _set_arr_highlighted(false);
            _update_rectangle_highlighted(id, false);
            _set_display_toolbar(false);
            _update_rectangle_toolbar_show(id, false);
            taskboard_rerender_func();

            _set_local_param_rectangle_active(false);
        }
        else
        {
            _set_prevent_rectangle_deactivation(false);
        }
    };
    ...
    const _toolbar_item_clicked_notif = (rectangle_tb_item_index) => {
        switch(rectangle_tb_item_index)
        {
    ...
    const _update_colour = (updated_hex_colour_val) => {
        _update_rectangle_colour(id, updated_hex_colour_val);
        taskboard_rerender_func();
    };
    ...
    const _get_toolbar_position = () => {
        ...
        let x = (rectangle_start_pos.x + rectangle_end_pos.x) / 2;
        let y = 0; 
        
        if(rectangle_start_pos.y < rectangle_end_pos.y)
        {
            y = rectangle_start_pos.y - toolbar_distance_top;
        }
        else
        {
            y = rectangle_end_pos.y - toolbar_distance_top;
        } 
    ...
    const _highlighter_drag_mouse_down = (drag_direction) => {
        _set_prevent_rectangle_deactivation(true);
        taskboard_rerender_func();
    ...
    const _on_mouse_down = (e) => {
        ...
        _update_rectangle_highlighted(id, true);
        _set_local_param_rectangle_active(true);
        
        _set_display_toolbar(true);
        _update_rectangle_toolbar_show(id, true);
        taskboard_rerender_func();
    };
    ...
    useEffect(() => {
        if (rectangle_root_ref.current && !rectangle_root_ref.current.contains(main_page_last_click_event_target)) {
            // clicked outside of the rectangle component, deactivate rectangle  
            if(local_param_rectangle_active === true)
            {
                _deactivate_rectangle(main_page_last_click_event_target);
            }
        }
        ...
        const _handle_mouse_up = (event) => {   
            if(is_dragging_hlighter === true)
            {
                if(selected_hlight_pos === LINE_HLIGHT_DRAG_POS.MID)
                {
                    ...
                    const mid_y = (rectangle_start_pos.y + rectangle_end_pos.y) / 2;
...
export default _draggable_rectangle;
